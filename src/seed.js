...
if (mode === 'test') {
  ...
  activeMentors.forEach((m, idx) => {
    ...
    if (m.can_technical) {
      try { addSlot.run(m.id, 'technical', tomorrow, sTime, eTime, 'Online', 'https://meet.google.com/tech-' + m.id); } catch (_) {}
    }
    if (m.can_hr) {
      try { addSlot.run(m.id, 'hr', dayAfter, sTime, eTime, 'Online', 'https://meet.google.com/hr-' + m.id); } catch (_) {}
    }
  });
}
